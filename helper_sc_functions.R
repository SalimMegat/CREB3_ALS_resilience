...
load.and.aggregate <- function(data.loc, run.ids, group.name, normalize=TRUE, n.delim = ">", n.field=2) {
  ...
  d10x.data <- sapply(run.ids, function(i){
    ...
    d10x <- Read10X_h5(paste0(file.path(data.loc,i,"outs/filtered_feature_bc_matrix/"),i,".h5"))
...
integrate.exp.list <- function(experiment.aggregated, nns=30,knns=50, knn.score=30, features.to.keep = NULL, future.workers=12, ref.index=NULL) {
  library(Seurat)
  library(future)
  plan("multisession", workers = future.workers)
  options(future.globals.maxSize= Inf)
